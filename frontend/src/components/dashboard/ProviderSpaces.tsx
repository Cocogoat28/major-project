// carfive/test_park/frontend/src/components/dashboard/ProviderSpaces.tsx
...
const ProviderLocations: React.FC = () => {
  ...
  const [slotModalSpace, setSlotModalSpace] = useState<Location | null>(null); // added
  ...
  // Slot modal handlers (added)
  const handleSlotModalOpen = (space: Location) => {
    setSlotModalSpace(space);
  };

  const handleSlotModalClose = () => {
    setSlotModalSpace(null);
  };

  const handleSlotUpdate = (updatedSpace: Location) => {
    setLocations((prev) =>
      prev.map((loc) => (loc._id === updatedSpace._id ? { ...loc, ...updatedSpace } : loc))
    );
  };
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-gray-200 dark:border-gray-700">
                    <div className="flex space-x-2">
                      ...
                      {/* NEW BUTTON: Slot Management */}
                      <button
                        onClick={() => handleSlotModalOpen(location)}
                        className="flex items-center px-3 py-1.5 rounded-lg text-sm font-medium bg-blue-50 text-blue-600 hover:bg-blue-100 dark:bg-blue-900/20 dark:text-blue-400 dark:hover:bg-blue-900/30 transition-colors duration-200"
                      >
