...
export default function Home() {
  ...
  // detect dark theme from html class (keeps map style consistent)
  ...
  if (loading) {
    return (
      <div className="h-[calc(100vh-64px)] flex items-center justify-center bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 dark:from-gray-900 dark:via-gray-900 dark:to-gray-900 animate-gradient-x">
        <LoadingScreen />
      </div>
    );
  }
  ...
  // ----- Render -----
  return (
    <div className={`h-[calc(100vh-64px)] relative bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 dark:from-gray-900 dark:via-gray-900 dark:to-gray-900 animate-gradient-x`}>
        ...
        <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center">
          ...
          <div className="flex-1 w-full max-w-2xl">
            <div className="relative">
              <MdSearch className="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400 text-xl z-10 dark:text-gray-300" />
              <input
                type="text"
                placeholder="Search for locations, areas, or landmarks..."
                value={searchQuery}
                onChange={(e) => handleSearchInputChange(e.target.value)}
                className="w-full pl-12 pr-12 py-4 bg-white/95 dark:bg-gray-800/80 backdrop-blur-sm border border-white/20 dark:border-gray-700 rounded-2xl shadow-2xl focus:outline-none focus:ring-4 focus:ring-blue-500/30 focus:border-transparent text-lg font-medium placeholder-gray-500 dark:placeholder-gray-400 transition-all duration-500 hover:shadow-3xl"
              />
              {searchQuery && (
                <button
                  onClick={() => {
                  ...
                  className="absolute right-4 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-all duration-300 hover:scale-110 dark:text-gray-300 dark:hover:text-gray-100"
                >
            ...
            {showSearchResults && searchResults.length > 0 && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-white/95 dark:bg-gray-800/90 backdrop-blur-sm rounded-2xl shadow-2xl border border-white/20 dark:border-gray-700 overflow-hidden z-30 max-h-48 overflow-y-auto animate-fade-in-up">
                {searchResults.map((result, index) => (
                  <button
                    key={index}
                    onClick={() => handleLocationSelect(result)}
                    className="w-full px-4 py-3 text-left hover:bg-blue-50 dark:hover:bg-gray-700 transition-all duration-300 border-b border-gray-100 last:border-b-0 flex items-center gap-3"
                  >
                    <MdLocationOn className="text-blue-500 text-xl flex-shrink-0" />
                    <div className="text-left">
                      <div className="font-medium text-gray-900 dark:text-gray-100 text-sm">{result.address.split(',')[0]}</div>
                      <div className="text-xs text-gray-500 dark:text-gray-400 truncate">{result.address.split(',').slice(1).join(',').trim()}</div>
                    </div>
                  </button>
                ))}
              </div>
          ...
          <div className="flex gap-3 items-center">
            ...
            <button
              ...
              className="bg-white/95 dark:bg-gray-800/80 backdrop-blur-sm px-6 py-4 rounded-2xl shadow-2xl hover:shadow-3xl transition-all duration-500 hover:bg-white dark:hover:bg-gray-700 flex items-center gap-3 border border-white/20 dark:border-gray-700 transform hover:scale-105 group"
            >
              <div className="relative">
              ...
              <div className="text-left">
                <div className="font-semibold text-gray-700 dark:text-gray-200 text-sm">Time Filter</div>
              ...
              {showTimeFilter ? <MdExpandLess className="text-gray-400 dark:text-gray-300" /> : <MdExpandMore className="text-gray-400 dark:text-gray-300" />}
            </button>
            ...
            >
              ...
              <div className="text-left">
                <div className="font-semibold text-gray-700 dark:text-gray-200 text-sm">Filters</div>
                <div className="text-xs text-gray-500 dark:text-gray-400">{getActiveFilterCount()} active</div>
              </div>
              {showFilters ? <MdExpandLess className="text-gray-400 dark:text-gray-300" /> : <MdExpandMore className="text-gray-400 dark:text-gray-300" />}
            </button>
            ...
            >
              ...
              <div className="text-left">
                <div className="font-semibold text-gray-700 dark:text-gray-200 text-sm">Parking List</div>
                <div className="text-xs text-gray-500 dark:text-gray-400">{filteredSpaces.length} spaces</div>
              </div>
              {showParkingList ? <MdExpandLess className="text-gray-400 dark:text-gray-300" /> : <MdExpandMore className="text-gray-400 dark:text-gray-300" />}
            </button>
          </div>
        </div>
      </div>
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 animate-fade-in">
    <div className="bg-white/95 dark:bg-gray-800/90 backdrop-blur-md rounded-2xl border border-gray-200 dark:border-gray-700 shadow-2xl p-6 w-full max-w-md transition-all duration-500 animate-fade-in-up">
      ...
      <div className="flex items-center justify-between mb-5">
        <h3 className="font-bold text-gray-800 dark:text-gray-100 text-lg flex items-center gap-2">
          ...
          Select Parking Time
          {(startTime || endTime) && (
            <span className="ml-2 bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300 text-xs px-2 py-1 rounded-full animate-pulse">
              Active
            </span>
          )}
        </h3>
        <button
          ...
          className="text-gray-400 hover:text-gray-600 transition-all duration-300 hover:scale-110 dark:text-gray-300 dark:hover:text-gray-100"
        >
          <MdClose className="text-xl" />
        </button>
      </div>
      ...
      <div className="mb-6">
        <label className="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">
          <MdAccessTime className="inline text-blue-500 mr-1" />
          Start
        </label>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <input
            type="date"
            min={getCurrentDateTimeRounded().split("T")[0]}
            max={getMaxDateTime().split("T")[0]}
            value={startTime ? startTime.split("T")[0] : ""}
            onChange={(e) => {
              const date = e.target.value;
              const time = startTime ? startTime.split("T")[1] : "09:00";
              setStartTime(`${date}T${time}`);
            }}
            className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-700 shadow-sm text-gray-700 dark:text-gray-100"
          />
          ...
          <select
            ...
            onChange={(e) => {
              ...
              const date = startTime ? startTime.split("T")[0] : getCurrentDateTimeRounded().split("T")[0];
              setStartTime(`${date}T${time}`);
            }}
            className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-700 shadow-sm text-gray-700 dark:text-gray-100"
          >
      ...
      <div className="mb-6">
        <label className="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">
          <MdAccessTime className="inline text-green-500 mr-1" />
          End
        </label>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <input
            type="date"
            min={startTime ? startTime.split("T")[0] : getCurrentDateTimeRounded().split("T")[0]}
            max={getMaxDateTime().split("T")[0]}
            value={endTime ? endTime.split("T")[0] : ""}
            onChange={(e) => {
              const date = e.target.value;
              const time = endTime ? endTime.split("T")[1] : "10:00";
              setEndTime(`${date}T${time}`);
            }}
            className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-green-500 bg-white dark:bg-gray-700 shadow-sm text-gray-700 dark:text-gray-100"
          />
          ...
          <select
            ...
            onChange={(e) => {
              const time = e.target.value;
              const date = endTime ? endTime.split("T")[0] : getCurrentDateTimeRounded().split("T")[0];
              setEndTime(`${date}T${time}`);
            }}
            className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-green-500 bg-white dark:bg-gray-700 shadow-sm text-gray-700 dark:text-gray-100"
            disabled={!startTime}
      ...
      <div className="flex gap-3">
        <button
          ...
          disabled={!(startTime && endTime && validateTimeSelection(startTime, endTime))}
          className="flex-1 bg-gradient-to-r from-blue-600 to-purple-600 text-white px-6 py-3 rounded-xl text-sm font-semibold hover:from-blue-700 hover:to-purple-700 transition-all duration-300 disabled:opacity-50 flex items-center justify-center gap-2 shadow-md"
        >
        ...
        <button
          ...
          className="px-6 py-3 border border-gray-300 dark:border-gray-600 rounded-xl text-sm font-semibold text-gray-700 dark:text-gray-100 hover:bg-gray-50 dark:hover:bg-gray-700 transition-all duration-300 bg-white dark:bg-gray-700 flex items-center justify-center gap-2"
        >
      ...
      {/* Info Footer */}
      <div className="mt-5 text-xs text-gray-500 dark:text-gray-400 text-center border-t border-gray-100 dark:border-gray-700 bg-blue-50 dark:bg-gray-800 rounded-lg p-3">
        <p className="font-medium mb-1">📅 Pick date and time separately</p>
        <p>Supports AM/PM display • Book up to 30 days in advance</p>
      </div>
    </div>
  </div>
)}
      ...
      {/* Filters Panel */}
      {showFilters && (
        <div className="absolute top-32 right-4 z-10 w-72 bg-white/95 dark:bg-gray-800/90 backdrop-blur-sm rounded-xl shadow-2xl border border-white/20 dark:border-gray-700 animate-fade-in-up">
          <div className="p-3 border-b border-gray-100 dark:border-gray-700">
            <div className="flex justify-between items-center">
              <h3 className="font-bold text-gray-800 dark:text-gray-100 text-base">Filter Parking</h3>
              <button
          ...
          <div className="p-3">
            <h4 className="font-semibold text-gray-700 dark:text-gray-200 text-sm mb-2">Amenities</h4>
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-2 p-2 rounded-lg transition-all duration-300 transform hover:scale-[1.02] ${
                      isActive
                        ? 'bg-gradient-to-r from-blue-50 to-purple-50 border-blue-200 shadow-md dark:bg-gray-700/60'
                        : 'bg-gray-50 dark:bg-gray-800/60 border-transparent hover:bg-gray-100 dark:hover:bg-gray-700'
                    }`}
                  >
                    <div
                    ...
                    <div className="flex-1 text-left">
                      <div className="font-medium text-gray-800 dark:text-gray-100 text-xs">{amenity.label}</div>
                      <div className="text-xs text-gray-500 dark:text-gray-400 truncate">{amenity.description}</div>
                    </div>
                    <div
      ...
      <div className="absolute top-20 right-4 z-30 animate-fade-in-down">
        <button
          ...
          className="bg-white/95 dark:bg-gray-800/80 backdrop-blur-sm p-4 rounded-2xl shadow-2xl hover:shadow-3xl transition-all duration-500 hover:bg-white dark:hover:bg-gray-700 border border-white/20 dark:border-gray-700 group transform hover:scale-110"
      ...
      <div className="relative h-full animate-fade-in">
        <Map
          ...
          mapStyle={isDark ? "mapbox://styles/mapbox/dark-v10" : "mapbox://styles/mapbox/streets-v12"}
        >
      ...
      {showParkingList && (
        <div
          ...
          className="absolute z-40 h-[480px] bg-white/95 dark:bg-gray-800/90 backdrop-blur-sm rounded-2xl shadow-2xl overflow-hidden border border-white/20 dark:border-gray-700 animate-fade-in-left"
        >
          <div
            onPointerDown={onPointerDownSidebar}
            className="p-4 border-b border-gray-100 dark:border-gray-700 bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-600 text-white cursor-grab select-none transition-all duration-300 hover:shadow-lg"
          ...
          <div className="p-4 border-b border-gray-100 dark:border-gray-700 bg-gradient-to-r from-blue-50 to-purple-50 dark:from-gray-800/60 dark:to-gray-700/50">
            <div className="flex gap-3">
              <button
