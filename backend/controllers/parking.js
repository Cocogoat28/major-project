...
// carfive/test_park/backend/controllers/parking.js

// ... (rest of file)

// Update Parking Slots
export const updateParkingSlots = async (req, res) => {
  try {
    if (!req.user) return res.status(401).json({ message: 'Not authorized, user missing' });

    const { totalSpots } = req.body;
    if (typeof totalSpots !== 'number' || totalSpots < 1) {
      return res.status(400).json({ message: 'totalSpots must be a number greater than 0' });
    }
    ...
    // Check if the new totalSpots is less than the number of currently booked slots
    const bookedCount = await BookedSlot.countDocuments({
      parkingSpace: id,
      isCancelled: { $ne: true },
      endTime: { $gt: new Date() } // Only consider future bookings
    });

    if (totalSpots < bookedCount) {
      return res.status(400).json({
        message: `Cannot reduce total slots to ${totalSpots}. There are currently ${bookedCount} future bookings.`
      });
    }

    // Update totalSpots and availableSpots
    parkingSpace.totalSpots = totalSpots;
    // Recalculate availableSpots based on the new total and current bookings
    parkingSpace.availableSpots = Math.max(0, totalSpots - bookedCount);

    await parkingSpace.save();

    const psObj = parkingSpace.toObject();
    // Re-apply photo URL normalization for response consistency
    if (Array.isArray(psObj.photos)) {
      psObj.photos = psObj.photos.map(p =>
        p?.startsWith('http' ) ? p : p?.startsWith('/') ? `${req.protocol}://${req.get('host')}${p}` : `${req.protocol}://${req.get('host')}/uploads/${p}`
      );
    ...
    return res.json({ message: 'Parking slots updated successfully', parkingSpace: psObj });
  } catch (error) {
    console.error('Error updating parking slots', error);
    return res.status(500).json({ message: 'Failed to update parking slots' });
  }
};